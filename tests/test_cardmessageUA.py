...
class TestMedicalTrainer:
    ...
    @pytest.mark.cardmessage_ua
    @pytest.mark.cardmessage
    @pytest.mark.smoke
    def test_cardmessageform_ua(self):
        print("*******")
        print("TEST THE CARD MESSAGE FORM IN UA INSTANCE PASSED")
        print("*******")
        cardmessage = CardMessage(self.driver)
        cardmessage.go_to_cardmessage()
        cardmessage.clicking_localization_dropdown()
        cardmessage.select_ua_in_localization_dropdown()
        ...
        screen_valid_name = f'cardmessage_UA_valid_{datetime.datetime.now()}.png'
        self.driver.get_screenshot_as_file(screen_valid_name)
        cardmessage.click_submit_button()
        screen_confirm_name = f'cardmessage_UA_confirm_{datetime.datetime.now()}.png'
