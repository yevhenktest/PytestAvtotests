...
class TestMedicalTrainer:
    ...
    @pytest.mark.cardmessage_en
    @pytest.mark.cardmessage
    @pytest.mark.smoke
    def test_cardmessageform_en(self):
        print("*******")
        print("TEST THE CARD MESSAGE FORM IN EN INSTANCE PASSED")
        print("*******")
        ...
        screen_valid_name = f'cardmessage_COM_valid_EN_{datetime.datetime.now()}.png'
        self.driver.get_screenshot_as_file(screen_valid_name)
        cardmessage.click_submit_button()
        screen_confirm_name = f'cardmessage_COM_confirm_EN_{datetime.datetime.now()}.png'
