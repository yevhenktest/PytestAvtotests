...
class TestMedicalTrainer:
    ...
    @pytest.mark.cardmessage_es
    @pytest.mark.cardmessage
    @pytest.mark.smoke
    def test_cardmessageform_es(self):
        print("*******")
        print("TEST THE CARD MESSAGE FORM IN ES INSTANCE PASSED")
        print("*******")
        cardmessage = CardMessage(self.driver)
        cardmessage.go_to_cardmessage()
        cardmessage.clicking_localization_dropdown()
        cardmessage.select_es_in_localization_dropdown()
        ...
        screen_valid_name = f'cardmessage_COM_valid_ES_{datetime.datetime.now()}.png'
        self.driver.get_screenshot_as_file(screen_valid_name)
        cardmessage.click_submit_button()
        screen_confirm_name = f'cardmessage_COM_confirm_ES_{datetime.datetime.now()}.png'
