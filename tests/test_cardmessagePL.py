...
class TestMedicalTrainer:
    ...
    @pytest.mark.cardmessage_pl
    @pytest.mark.cardmessage
    @pytest.mark.smoke
    def test_cardmessageform_pl(self):
        print("*******")
        print("TEST THE CARD MESSAGE FORM IN PL INSTANCE PASSED")
        print("*******")
        cardmessage = CardMessage(self.driver)
        cardmessage.go_to_cardmessage()
        cardmessage.clicking_localization_dropdown()
        cardmessage.select_pl_in_localization_dropdown()
        cardmessage.click_confirm_popup_window()
        ...
        screen_valid_name = f'cardmessage_COM_valid_PL_{datetime.datetime.now()}.png'
        self.driver.get_screenshot_as_file(screen_valid_name)
        cardmessage.click_submit_button()
        screen_confirm_name = f'cardmessage_COM_confirm_PL_{datetime.datetime.now()}.png'
