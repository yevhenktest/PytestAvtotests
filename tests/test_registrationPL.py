...
class TestMedicalTrainer:
    ...
    def test_medicalform_pl(self):
        print("*******")
        print("TEST THE REGISTRATION FORM IN PL INSTANCE PASSED")
        print("*******")
        registration = Registration(self.driver)
        registration.go_to_registrationform()
        registration.clicking_localization_dropdown()
        registration.select_pl_in_localization_dropdown()
        registration.click_confirm_popup_window()
        ...
        registration.input_email('apl@gmail.com')
        registration.input_password('Test12345')
        registration.input_confirmpassword('Test12345')
        screen_valid_name = f'registration_COM_valid_PL_{datetime.datetime.now()}.png'
        self.driver.get_screenshot_as_file(screen_valid_name)
        registration.click_submit_button()
        screen_confirm_name = f'registration_COM_confirm_PL_{datetime.datetime.now()}.png'
